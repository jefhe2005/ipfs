...
func (dht *IpfsDHT) HandleMessage(ctx context.Context, mes msg.NetMessage) (msg.NetMessage, error) {
	...
	u.DOut("[peer: %s]\nGot message type: '%s' [id = %x, from = %s]\n",
		...
		pmes.GetId(), mPeer.ID.Pretty())

	// get handler for this msg type.
	var resp *PBDHTMessage
	...
	// dispatch handler.
	return handler(mPeer, pmes)
}

// dhthandler specifies the signature of functions that handle DHT messages.
type dhtHandler func(*peer.Peer, *PBDHTMessage) (*PBDHTMessage, error)

func (dht *IpfsDHT) handlerForMsgType(t PBDHTMessage_MessageType) dhtHandler {
	switch t {
...
func (dht *IpfsDHT) handleGetValue(p *peer.Peer, pmes *PBDHTMessage) (*PBDHTMessage, error) {
